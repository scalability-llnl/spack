...
.. _cmd-spack-ci-prebuild:

^^^^^^^^^^^^^^^^^^^^^
``spack ci prebuild``
^^^^^^^^^^^^^^^^^^^^^

This sub-command corresponds to a single job in the generated pipeline and
is responsible for preparing a ``spack install`` command line to build a
single spec in the DAG.  The command line is saved in a one-line bash script
called ``install.sh``, which by default is run by the job and also exported
as an artifact to aid in reproducing the build outside of the CI environment.
...
``.gitlab-ci.yml`` file as ``variables`` at either the job or pipeline level.
...
.. code-block:: yaml
   ...
   - spack env activate --without-view .
   - spack ci prebuild
   - ./install.sh || export SPACK_INSTALL_EXIT_ERROR=$?
   - spack ci postbuild
...
.. _cmd-spack-ci-postbuild:

^^^^^^^^^^^^^^^^^^^^^^
``spack ci postbuild``
^^^^^^^^^^^^^^^^^^^^^^

This sub-command is responsible for pushing buildcaches to binary mirror(s),
possibly interacting with a CDash instance to relate the spec build of the
current job with the builds of its dependencies.
