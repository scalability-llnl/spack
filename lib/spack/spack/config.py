...
class Configuration:
    ...
    @_config_mutator
    def push_scope(self, scope: ConfigScope, depth: int = 0) -> None:
        ...
        if includes:
            ...
            for path in reversed(include_paths):
                ...
                if depth + 1 > MAX_RECURSIVE_INCLUDES:  # make sure we're not recursing endlessly
                ...
                # record this inclusion so that pop_scope() can use it
                scope.included_scopes.append(included_scope.name)
                self.push_scope(included_scope, depth=depth + 1)

        self.scopes[scope.name] = scope

    def _remove_included_scopes(self, scope: ConfigScope) -> None:
        """Helper for pop_scope and remove_scope. Transitively removes included scopes."""
