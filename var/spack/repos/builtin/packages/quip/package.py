...
class Quip(MakefilePackage):
    ...
    def setup_build_environment(self, env):
        ...
        env.set("MATH_LINKOPTS",    f"{self.spec['lapack'].libs.ld_flags} {self.spec['blas'].libs.ld_flags}")
    ...
    def edit(self, spec, prefix):
        ...
        makefile.filter(
            r"^\s*GIT_VERSION\s*:=.*",
            f"GIT_VERSION := -D'GIT_VERSION=\"{gitversion}\"'"
        )
        #
    ...
    def build(self, spec, prefix):
        ...
        make_directory = join_path(self.build_directory, "spack-src")
